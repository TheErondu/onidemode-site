    ...
    <form method="POST" action="{{ route('admin.pages.manage.update', $page) }}">
        ...
        <div class="card my-4">
            ...
            <div class="card-body">
                ...
                @foreach ($page['content']['sections'][0]['videos'] as $index => $video)
                <div class="form-group">
                    <label for="video_slide{{ $index }}">Slideshow video {{ $index + 1 }} Source</label>
                    <input type="text" class="form-control" name="sections[0][videos][{{ $index }}][src]" value="{{ $video['src'] }}">
                </div>
            @endforeach
            </div>
        </div>
